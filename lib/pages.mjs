...
function logo({ dark = false } = {}) {
  const src = dark ? '/byc-footer.png' : '/byclogo-2.png';
...
function packageCard(item, { compact = false } = {}) {
  ...
  const providerLogo = item.is_featured ? '/voda-2.png' : '/voda-1.png';
  return `<article class="package-card ${item.is_featured ? 'featured' : ''}" data-package="${escapeHtml(item.slug)}" data-package-choice="${escapeHtml(item.slug)}" tabindex="0" role="button" aria-label="Choose ${escapeHtml(item.name)}">
    ...
    <img class="package-provider-logo" src="${providerLogo}" alt="Vodacom Business" width="112" height="62" loading="lazy">
...
function homePage({ config, packages }) {
  const content = `<section class="hero">
    ...
    <div class="hero-inner container-wide">
      ...
      <aside class="partner-card"><p>Connectivity partner</p><div class="partner-lockup"><img class="partner-provider-logo" src="/voda-1.png" alt="Vodacom Business" width="190" height="106"><span class="partner-divider"></span><img class="group-logo" src="/byclogo-2.png" alt="Backyard Connect" width="190" height="62"></div></aside>
    </div>
  </section>
