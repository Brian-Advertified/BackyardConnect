...
function logo({ dark = false } = {}) {
  ...
  return `<a class="brand" href="/" aria-label="Backyard Connect home"><img src="${src}" alt="Backyard Connect" width="570" height="150"><small>From the BackyardFinance group</small></a>`;
}
...
function packageCard(item, { compact = false } = {}) {
  ...
  return `<article class="package-card ${item.is_featured ? 'featured' : ''}" data-package="${escapeHtml(item.slug)}" data-package-choice="${escapeHtml(item.slug)}" tabindex="0" role="button" aria-label="Choose ${escapeHtml(item.name)}">
    ...
    <img class="package-provider-logo" src="/assets/vodacom-business.svg" alt="Vodacom Business" width="112" height="34" loading="lazy">
...
function referralForm(packages, { heading = 'Tell us about your property', description = 'Share a few details about your property and a connectivity specialist will contact you.' } = {}) {
  return `<section id="referral-form" class="referral-shell" aria-labelledby="referral-title">
    ...
    <form class="referral-form" data-referral-form novalidate>
      ...
      <label class="consent-field"><input type="checkbox" name="consent" required><span>I consent to BackyardConnect sharing my details with its current connectivity partner so that I can be contacted about this enquiry.<span class="field-error" data-error-for="consent"></span></span></label>
...
function homePage({ config, packages }) {
  const content = `<section class="hero">
    ...
    <div class="hero-inner container-wide">
      ...
      <aside class="partner-card"><p>Current connectivity partner</p><div class="partner-lockup"><img src="/assets/vodacom-business.svg" alt="Vodacom Business" width="190" height="58"><span class="partner-divider"></span><span class="group-logo"><span class="brand-mark">${icon('home')}</span><strong>Backyard<br>Finance</strong></span></div><p class="group-caption">From the BackyardFinance group</p></aside>
    </div>
  </section>
...
function packagesPage({ config, packages }) {
  const content = `<section class="page-hero compact"><div class="container"><p class="eyebrow">Current connectivity partner: Vodacom Business</p><h1>Connectivity options for rental properties.</h1><p>Use the room ranges as a starting guide. Vodacom confirms coverage, eligibility, pricing and final package suitability.</p></div></section>
...
function faqPage({ config }) {
  const questions = [
    ['Does BackyardConnect provide the internet service?', 'BackyardConnect helps property owners explore connectivity options. Vodacom Business is the current connectivity partner and confirms coverage, eligibility, pricing and fulfilment.'],
